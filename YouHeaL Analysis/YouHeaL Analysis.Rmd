...
get_data_timelineS <- function(){
  ...
  rbind(cohort1, cohort2, cohort3) %>% 
    ...
    mutate(
      ...
      # make variable with increasing width for subsequent cohorts
      #   note: there were bugs with legend widths, so decided against this
      # width_cohort = case_when(
      #   cohort == "Cohort 1" ~ 3,
      #   cohort == "Cohort 2" ~ 6,
      #   cohort == "Cohort 3" ~ 9
      # )
...
plot_timelineS <- function(data){
  ...
  ggtimeline <- data %>% 
    timelineG(start = "start", end = "end", names = "cohort", phase = "phase", width = 6) +
    theme(
      legend.position = c(0, 0.98),
      legend.direction = "horizontal",
      legend.box.background = element_rect(size = 10, color = "white"),
...
param_timeline_width  <- 2400
param_timeline_height <- 1400
param_timeline_resolution <- 300

# save copy of timeline overall
ggtimeline_all <- data_timelineS %>% plot_timelineS()
dev.copy(png, "YouHeaL_ggtimeline_all.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline_all
dev.off()
...
dev.copy(png, "Timeline/YouHeaL_ggtimeline1.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline1
dev.off()

##timeline2
ggtimeline2 <- data_timelineS %>% dplyr::filter(start < as.Date("2019-01-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline2.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline2
dev.off()

##timeline3
ggtimeline3 <- data_timelineS %>% dplyr::filter(start < as.Date("2019-06-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline3.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline3
dev.off()

##timeline4
ggtimeline4 <- data_timelineS %>% dplyr::filter(start < as.Date("2020-01-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline4.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline4
dev.off()

##timeline5
ggtimeline5 <- data_timelineS %>% dplyr::filter(start < as.Date("2020-06-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline5.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline5
dev.off()

##timeline6
ggtimeline6 <- data_timelineS %>% dplyr::filter(start < as.Date("2021-01-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline6.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline6
dev.off()

##timeline7
ggtimeline7 <- data_timelineS %>% dplyr::filter(start < as.Date("2021-06-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline7.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline7
dev.off()

##timeline8
ggtimeline8 <- data_timelineS %>% dplyr::filter(start < as.Date("2022-01-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline8.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline8
dev.off()

##timeline9
ggtimeline9 <- data_timelineS %>% dplyr::filter(start < as.Date("2022-06-01")) %>% plot_timelineS()
dev.copy(png, "Timeline/YouHeaL_ggtimeline9.png", width = param_timeline_width, height = param_timeline_height, res = param_timeline_resolution)
ggtimeline9
dev.off()
```
...
This chunk scores the [General Self-Efficacy (GSE) Scale](https://www.psytoolkit.org/survey-library/generalized-self-efficacy-gse.html).  
...
test_results_efficacy <- as.data.frame(rbind(
  
  test_diff("total_efficacy")
...
test_results_efficacy %>%
  ...
  xlab("") +
  ylim(c(0,2)) +
  ylab("\nDifference From Baseline to Follow-Up (Points, Range 10 to 40)") +
  ggtitle("Self-Efficacy: Can You Effectively Reach Your Goals?")
...
test_results_empower %>%
  ...
  xlab("") +
  ylab("\nDifference From Baseline to Follow-Up (Points, Range 12 to 48)") +
  ylim(c(0,2)) +
...
test_results_diet %>%
  ...
  xlab("") +
  ylab("\nDifference From Baseline to Follow-Up (Count)") +
  ylim(c(-0.13,1)) +
...
test_results_physical %>%
  ...
  xlab("") +
  ylab("\nDifference From Baseline to Follow-Up (Days)") +
...
test_results_physical %>%
  ...
  scale_y_continuous(limits = c(-60, 0), breaks = seq(-60, 0, 20)) +
  labs(caption = "* p-value < 0.05") +
  theme(
  ...
  xlab("") +
  ylab("\nDifference From Baseline to Follow-Up (Minutes)") +
  ggtitle("On an Average School Day, How Many Minutes Did You...")
...
test_results_anthro %>%
  ...
  geom_bar(stat = "identity", fill = col_usc_red, width = 0.5) +
  coord_flip() +
  scale_y_continuous(limits = c(-0.05, 1), breaks = seq(0, 1, 0.2), labels = scales::percent_format(accuracy = 1)) +
  ...
  xlab("") +
  ylab("\n% Difference From Baseline to Follow-Up (n = 30)") +
  ggtitle("Percent Overweight by Body Measurements")
...
```{r, message=FALSE}
plot_complete_items(quo(total_efficacy)) + ggtitle(label(analytic_cohort[["total_efficacy"]]))
```

###Empowerment

```{r, message=FALSE}
plot_complete_items(quo(empower_composite)) + ggtitle(label(analytic_cohort[["empower_composite"]]))
plot_complete_items(quo(empower_individual)) + ggtitle(label(analytic_cohort[["empower_individual"]]))
plot_complete_items(quo(empower_community)) + ggtitle(label(analytic_cohort[["empower_community"]]))
plot_complete_items(quo(empower_organization)) + ggtitle(label(analytic_cohort[["empower_organization"]]))
```
