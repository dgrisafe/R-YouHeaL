...
Try creating a timeline using the [timelineS package](https://github.com/daheelee/timelineS).
...
analytic_cohort_nomiss <- analytic_cohort %>% 
  ...
  mutate(
    ...
    miss_ind = factor(ifelse(test = any(is.na(phase)), yes = 1, no = 0), levels = c(0, 1), labels = c("Participants", "Lost to Follow-Up")),
    age = as.double(age),
    obesity_TSF = obesity_TSF[,1],
    obesity_WC = obesity_WC[,1],
...
table1_nomiss <- CreateTableOne(data = analytic_cohort_nomiss_baseline, strata = "miss_ind", vars = table1_nomiss_vars, factorVars = table1_nomiss_categorical, test = TRUE, includeNA = FALSE)
...
table1_nomiss_html %>% kableone(digits = 3) %>%
  ...
  pack_rows("Anthropometric Measurements", 16, 19)
```
...
# get baseline demographics
  ...
  geom_hline(yintercept = c(.25*30, .50*30, .75*30, 1*30), linetype = 2, alpha = 0.4) +
  geom_bar(aes(y = yes_only), alpha = 1, stat = "identity", fill = "#990000") +
  annotate("text", x = 0.7, y = .25*30+1.5, label =  "25%", alpha = 0.4, size = 3.5) +
  annotate("text", x = 0.7, y = .50*30+1.5, label =  "50%", alpha = 0.4, size = 3.5) +
  annotate("text", x = 0.7, y = .75*30+1.5, label =  "75%", alpha = 0.4, size = 3.5) +
  annotate("text", x = 0.7, y = 1.0*30+2, label =  "100%", alpha = 0.4, size = 3.5) +
  scale_y_continuous() +
  coord_flip() +
  ggtitle("Demographic and Clinical Characteristics") +
  ylab("\nAnalytic Cohort (n = 30)") +
  xlab("") +
  theme(
    plot.title = element_text(hjust = 0.5),
...
#Longitudinal Data Analysis{.tabset}
